...
const ResetPwd = async (payload) => {
    const {userId, token, user_password} = payload
    ...
    await Cache.setDataIfNotExist(user_password, {"passwordHache": (await hashedPwd(user_password))})
    // recupération du mot de passe haché et comparaison
    const pwdData = await Cache.getTokenData(user_password)
    // constitution du payload de mise à jour
    const updatePayload = {...User.toJSON(), user_password : pwdData.passwordHache};
    try {
