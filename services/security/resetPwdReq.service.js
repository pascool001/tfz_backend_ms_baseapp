...
const ResetPwdRequest = async (user_email) => {
    // Extraction du user concerné
    const User = await UserRepo.findOne({user_email})
    ...
    let TokenOrOtp = (User.user_type === "WEB") ? crypto.randomBytes(32).toString("hex"): (await genOtp());
    ...
    const emailOrSmsResult = (User.user_type === "WEB") ?
    (await sendEmail(
        User.user_email, 
        "Réinitialisation du mot de passe", 
        emailPayload, "./template/requestResetPassword.handlebars",
        `Un lien de re-initialisation de mot de passe a été envoyé à l'addresse: ${user_email} `
