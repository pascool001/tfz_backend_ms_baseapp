...
const getAll = async (request, response) => {
    ...
    // console.log("CountryList: ", List.length)
...
const getCountryByCode = async (request, response) => {
    const code = request.path_parameters.code;
    const countryData = await CountryRepo.findOne({country_code: code});
    return response.json( {...countryData.toJSON()})
}
